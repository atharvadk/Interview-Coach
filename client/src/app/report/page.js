...
function ReportContent() {
    ...
    return (
        <div className="flex-1 flex items-center justify-center min-h-[60vh]">
           <div className="text-center">
             ...
             <p className="text-slate-400 mb-6">We couldn't find the session data or it is still processing.</p>
  ...
  // 3. Prepare Chart Data (Using 'questions' list from backend schema)
  const chartData = report.questions.map((q, i) => ({
    name: `Q${i + 1}`,
    score: q.composite_score, // Aligned with QuestionReport schema
    semantic: q.semantic_score,
    keywords: q.keyword_score
  }));
    ...
    <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
         <div className="glass-card rounded-2xl p-6 flex flex-col items-center justify-center text-center col-span-1 border-emerald-500/20 bg-emerald-500/5">
            <div className="text-5xl font-black text-emerald-400 mb-2">{report.average_score} <span className="text-xl text-emerald-600">/ 10</span></div>
            <div className="text-emerald-500 font-semibold mb-4 uppercase tracking-widest text-sm">Overall Score</div>
            <div className="flex items-center gap-4 text-sm">
              <div className="flex items-center gap-1 text-slate-300">
                <span className="font-medium">Domain:</span> <span className="bg-slate-800 px-2 py-0.5 rounded uppercase">{report.domain}</span>
              </div>
              <div className="flex items-center gap-1 text-slate-300">
                <span className="font-medium">Questions:</span> <span className="bg-slate-800 px-2 py-0.5 rounded">{report.total_questions}</span>
              </div>
            </div>
         </div>
         ...
         <div className="glass-card rounded-2xl p-6 col-span-1 md:col-span-2">
           ...
           <p className="text-slate-300 leading-relaxed mb-6">{report.overall_feedback}</p>
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
             ...
             <div>
                ...
                <ul className="space-y-2">
                  {report.areas_to_improve?.map((imp, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-slate-300">
        ...
        <div className="h-[250px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              />
              <Line type="monotone" dataKey="score" stroke="#10b981" strokeWidth={3} name="Overall" />
              <Line type="monotone" dataKey="semantic" stroke="#8b5cf6" strokeWidth={2} strokeDasharray="5 5" name="Semantic" />
              <Line type="monotone" dataKey="keywords" stroke="#3b82f6" strokeWidth={2} strokeDasharray="5 5" name="Keywords" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold font-mono ${
                  q.composite_score >= 7 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'
                }`}>
                  {q.composite_score}
                </div>
                <div>
                  <h4 className="text-white font-medium line-clamp-1 pr-4">Q{q.question_number}: {q.question}</h4>
                </div>
              </div>
              <div>
                {expandedQs[i] ? <ChevronUp className="w-5 h-5 text-slate-400" /> : <ChevronDown className="w-5 h-5 text-slate-400" />}
              </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-slate-300 italic text-sm border-l-2 border-slate-500 pl-3 py-1 bg-slate-500/5 rounded-r">
                        "{q.user_answer}"
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                       <div className="bg-slate-800/50 p-3 rounded-lg">
                         ...
                         <div className="font-bold text-purple-400">{q.semantic_score}/10</div>
                       </div>
                       <div className="bg-slate-800/50 p-3 rounded-lg">
                         <div className="text-xs text-slate-400 mb-1">Keywords Match</div>
                         <div className="font-bold text-blue-400">{q.keyword_score}/10</div>
                       </div>
                       <div className="bg-slate-800/50 p-3 rounded-lg">
                         <div className="text-xs text-slate-400 mb-1">Grammar</div>
                         <div className="font-bold text-white">{q.grammar_score}/10</div>
                       </div>
                       <div className="bg-slate-800/50 p-3 rounded-lg">
                         <div className="text-xs text-slate-400 mb-1">Emotion Analysis</div>
                         <div className="font-bold text-slate-200 capitalize">{q.dominant_emotion || "Steady"}</div>
                       </div>
                    </div>
