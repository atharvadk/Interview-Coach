...
// ── POST /api/session/upload-resume ──────────────────────────────────────────
...
router.get("/stats", authMiddleware, async (req, res) => {
  try {
    ...
    // Best domain calculation
    const domainScores = {};
    const domainCounts = {};

    sessions.forEach((s) => {
      const domain = s.faceAnalysis?.domain || "unknown";
      const scores = s.questions.map((q) => q.score).filter((x) => x != null);
    ...
    // Chart data — last 10 sessions oldest to newest
...
// ── GET /api/session/:id ──────────────────────────────────────────────────────
// IMPORTANT: must be AFTER /stats and /history
